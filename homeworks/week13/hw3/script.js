...
/* 刪除留言 */
function del_post(a) {
    const conf = confirm('是否刪除留言');
    if (conf == true) {
        const mess_id = a.target.id.split('_')[1];
        const url = `./handleDeletePost.php?mess_id=${mess_id}`;
